...
def send_email(name, email, phone, message):
    user_input = f"Subject:New Message\n\nName: {name}\nEmail: {email}\nPhone: {phone}\nMessage: {message}"
...
def login():
    ...
    if form.validate_on_submit() and request.method == "POST":
        ...
        if not get_user:
            flash("That email does not exist, please try again or sign up to register.")
            return redirect(url_for('login'))

        check_password = check_password_hash(pwhash=get_user.password, password=user_password)
        if check_password:
            login_user(get_user)
            return redirect(url_for("get_all_posts"))
        else:
